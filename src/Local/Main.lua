    ...
    local function ensureSelfChams()
        ...
        if State.Local.SelfChams and ch then
            if not selfHighlight or selfHighlight.Parent~=ch then
                ...
                selfHighlight.FillColor=BLUE; selfHighlight.OutlineColor=Color3.fromRGB(255,255,255); selfHighlight.FillTransparency=.7; selfHighlight.OutlineTransparency=.1; selfHighlight.Parent=ch
            end
    ...
    local function ensureTrail()
        ...
        if State.Local.Trail and root then
            if not trail or not trail.Parent then
                ...
                trail=Instance.new("Trail"); trail.Name="LvkHubTrail"; trail.Attachment0=trailA; trail.Attachment1=trailB; trail.Lifetime=.35; trail.MinLength=.1; trail.FaceCamera=true; trail.Color=ColorSequence.new(BLUE); trail.Parent=root
            end
            trail.Enabled=true
        ...
        for _,d in ipairs(cam:GetDescendants()) do
            if d:IsA("BasePart") and not (LP.Character and d:IsDescendantOf(LP.Character)) then
                ...
                if n:find("gun",1,true) or n:find("weapon",1,true) or n:find("viewmodel",1,true) or n:find("arm",1,true) then
                    ...
                    if State.Local.GunChams then d.Material=Enum.Material.ForceField; d.Color=BLUE else local o=gunOriginal[d]; if o then d.Material=o.Material; d.Color=o.Color end end
                end
            end
        end
    end
